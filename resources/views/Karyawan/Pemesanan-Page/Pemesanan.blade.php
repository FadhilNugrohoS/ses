    ...
    <table class="table table-bordered table-striped table-hover">
        <thead style="text-align: center">
            <div class="d-flex flex-row-reverse px-2 py-2" >
                <a type="button" class="btn btn-primary btn-lg" href="{{ route ('pemesanan.create') }} ">Tambah</a>
            </div>
          <tr>
            <th rowspan="2">No</th>
        ...
        <tbody>
          ...
          ?>
            ...
            <tr style="text-align: center">
                ...
                <td>
                    @if($pesan->status == 0)
                      <a href="{{route('karyawan.pemesanan.proses',$pesan->id)}}"><button button type="submit" class="btn btn-sm btn-primary">Verifikasi</button></a>
                    @elseif($pesan->status == 1)
                ...
                <td>
                  @if($pesan->status == 0)
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                        action="{{ route('pemesanan.destroy', $pesan->id) }}" method="POST">
